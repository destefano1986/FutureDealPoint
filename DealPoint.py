...
#如下函数参数需优化
para = zhichengwei_point(20, 0.5, 3)
...
for i in range(0, len(para)):
    ...
    if (type == 'zcw') & (direction == 'bear'):
        ...
        if buffer > 0:
            for j in range(0, int2):
                maximum = (data.loc[para.loc[i, 'index'] + j + 1, 'high'] - para.loc[i, 'point']) / para.loc[i, 'point']
                if maximum > tmp:
                    tmp = maximum
        ...
        if buffer > 0:
            for k in range(0, int3):
                money = (para.loc[i, 'point'] - data.loc[para.loc[i, 'index'] + k + 1, 'low']) / data.loc[para.loc[i, 'index'] + k + 1, 'low']
                if money > tmpcc:
                    tmpcc = money
    ...
    if (type == 'zcw') & (direction == 'bull'):
        ...
        if buffer > 0:
            for j in range(0, int2):
                maximum = (para.loc[i, 'point'] - data.loc[para.loc[i, 'index'] + j + 1, 'low']) / data.loc[para.loc[i, 'index'] + j + 1, 'low']
                if maximum > tmp:
                    tmp = maximum
        if tmp <= 0.005 and tmp > 0:
            threshold05 = threshold05 + 1
        ...
        if buffer > 0:
            for k in range(0, int3):
                money = (data.loc[para.loc[i, 'index'] + k + 1, 'high'] - para.loc[i, 'point']) / para.loc[i, 'point']
                if money > tmpcc:
                    tmpcc = money
        if tmpcc <= 0.01 and tmpcc > 0:
            thres1 = thres1 + 1
    ...
    if (type == 'zlw') & (direction == 'bear'):
        ...
        if buffer > 0:
            for k in range(0, int3):
                money = (data.loc[para.loc[i, 'index'] + k + 1, 'low'] - para.loc[i, 'point']) / para.loc[i, 'point']
                if money > tmpcc:
                    tmpcc = money
        if tmpcc <= 0.01 and tmpcc > 0:
            thres1 = thres1 + 1
